...
const EXTERNAL_OR_SPECIAL_REGEX = /^(?:[a-z][a-z\d+\-.]*:)?\/\/|^(#|mailto:|tel:)/i;
...
export function Link({ href, style, children, ...rest }) {
  const formattedHref =
    typeof href === "string" && href.trim().length > 0 ? formatURL(href) : undefined;

  return (
    <a href={formattedHref} style={style ?? { color: "var(--palette-dark-blue)" }} {...rest}>
      {children}
    </a>
  );
}
